...
class RegistroVacinacaoDAO {
    ...
    public function Remover($id_vacinacao) {

        $sql = "DELETE FROM registrovacinacao WHERE id_vacinacao = ?";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param("i", $id_vacinacao); 
        return $stmt->execute();
    }

    public function Atualizar(RegistroVacinacao $registrovacinacao) {

        $sql = "UPDATE registrovacinacao SET id_animal=?, nome_vacina=?, data_aplicacao=?, aplicador=?, dose=? WHERE id_vacinacao=?";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param("issssi", $registrovacinacao->id_animal, $registrovacinacao->nome_vacina, $registrovacinacao->data_aplicacao, $registrovacinacao->aplicador, $registrovacinacao->dose, $registrovacinacao->id_vacinacao);
        return $stmt->execute();
    }

    public function BuscarPorId($id_vacinacao){
        
        $sql = "SELECT * FROM registrovacinacao WHERE id_vacinacao = ?"; 
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param("i", $id_vacinacao);
        $stmt->execute();
        $result = $stmt->get_result();
